...
case object ConnectorJDriver extends MysqlDriver {
  ...
  override val url = s"jdbc:mysql://localhost:$Port/$DataBase?user=$UserName&password=$Password&cacheServerConfiguration=true&createDatabaseIfNotExist=false"
}

case object MariaDbDriver extends MysqlDriver {
  override val name = "MariaDB"

  override val url = s"jdbc:mariadb://localhost:$Port/$DataBase?user=$UserName&password=$Password&fastConnect=true&tcpAbortiveClose=true"
}

case object DrizzleDriver extends MysqlDriver {
  override val name = "Drizzle"

  override val url = s"jdbc:drizzle://$UserName:$Password@localhost:$Port/$DataBase"
}
